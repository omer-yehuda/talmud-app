import type { PageInfo, PageContent, StudyPageContent, ComparisonPageContent } from "@/types";
...
export function getPageContent(id: string): PageContent | undefined {
  ...
  if (page.type === "study") {
    ...
    const topic = studyTopics.find((t) => t.id === pageNum);
    if (!topic) return undefined;

    return {
      info: { ...page, type: "study" },
      studyTopic: topic,
    } as StudyPageContent;
  }

  return {
    info: { ...page, type: "comparison" },
    comparisonItems,
  } as ComparisonPageContent;
}
